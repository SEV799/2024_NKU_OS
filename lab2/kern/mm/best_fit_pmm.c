 ...
 *              The struct 
  is used for the management of free mem blocks. At first you should
    ...
    if (list_empty(&free_list)) {
        ...
        while ((le = list_next(le)) != &free_list) {
            ...
            if (base < page) { // 插入到第一个比base大的页前面
                 ...
                 list_add(le, &(base->page_link)); // 到达链表尾部则插入尾部
            }
        }
    }
}
/* 根据best-fit策略分配内存页，返回分配的内存页地址 */
static struct Page *
best_fit_alloc_pages(size_t n) {
    ...
    // 遍历空闲链表，查找最适合的空闲块（大小最接近n）
     ...
     if (page != NULL) {
         ...
         list_del(&(page->page_link)); // 从空闲链表中删除分配的块

         // 如果空闲块大小大于请求大小，将剩余部分设为新的空闲块
         if (page->property > n) {
             struct Page *p = page + n;
             p->property = page->property - n; // 剩余大小
             SetPageProperty(p); // 标记剩余块
             list_add(prev, &(p->page_link)); // 插入到链表
         }
         nr_free -= n;          // 更新空闲页数量
         ClearPageProperty(page); // 标记已分配
     }
...
best_fit_free_pages(struct Page *base, size_t n) {
    ...
    if (le != &free_list) {
        ...
        if (p + p->property == base) { // 如果前块与当前块相连
             ...
             ClearPageProperty(base); // 清除当前块标志
             list_del(&(base->page_link)); // 删除当前块
             base = p; // 继续合并
         }
    }
