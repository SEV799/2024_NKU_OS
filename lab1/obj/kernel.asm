...
void interrupt_handler(struct trapframe *tf) {
    ...
    802003ce:	06f76b63          	bltu	a4,a5,80200444 <interrupt_handler+0x80>
...
void interrupt_handler(struct trapframe *tf) {
    ...
    8020041e:	601c                	ld	a5,0(s0)
             // 当计数器加到100的时候，打印信息
             if (num >= TICK_NUM) {
    80200420:	06300713          	li	a4,99
             num++;
    80200424:	0785                	addi	a5,a5,1
    80200426:	e01c                	sd	a5,0(s0)
             if (num >= TICK_NUM) {
    80200428:	601c                	ld	a5,0(s0)
    8020042a:	00f76e63          	bltu	a4,a5,80200446 <interrupt_handler+0x82>
                 print_ticks();
                 num = 0;  // 重置计数器
             }
             // 判断打印次数，当打印次数为10时，关机
    ...
    80200430:	cb85                	beqz	a5,80200460 <interrupt_handler+0x9c>
            break;
        default:
            print_trapframe(tf);
            break;
    ...
    8020043e:	b0650513          	addi	a0,a0,-1274 # 80200f40 <etext+0x524>
    80200442:	b13d                	j	80200070 <cprintf>
            print_trapframe(tf);
    80200444:	b705                	j	80200364 <print_trapframe>
    cprintf("%d ticks\n", TICK_NUM);
    80200446:	06400593          	li	a1,100
    8020044a:	00001517          	auipc	a0,0x1
    8020044e:	ae650513          	addi	a0,a0,-1306 # 80200f30 <etext+0x514>
    80200452:	c1fff0ef          	jal	ra,80200070 <cprintf>
                 num = 0;  // 重置计数器
    80200456:	00004797          	auipc	a5,0x4
    8020045a:	bc07b123          	sd	zero,-1086(a5) # 80204018 <num>
    8020045e:	bfc1                	j	8020042e <interrupt_handler+0x6a>
}
...
void exception_handler(struct trapframe *tf) {
    ...
    8020048c:	be5ff0ef          	jal	ra,80200070 <cprintf>
    	    cprintf("EPC: 0x%08x - Faulting Instruction Address\n", tf->epc);  // (2) 输出异常指令地址 
    80200490:	10843583          	ld	a1,264(s0)
    80200494:	00001517          	auipc	a0,0x1
    80200498:	b1c50513          	addi	a0,a0,-1252 # 80200fb0 <etext+0x594>
    8020049c:	bd5ff0ef          	jal	ra,80200070 <cprintf>
            tf->epc += 4;  // (3) 更新 tf->epc 寄存器，跳过非法指令
    ...
    802004d6:	ade50513          	addi	a0,a0,-1314 # 80200fb0 <etext+0x594>
    802004da:	b97ff0ef          	jal	ra,80200070 <cprintf>
    	    tf->epc += 2;  // (3) 更新 tf->epc 寄存器，跳过断点指令
...
printnum(void (*putch)(int, void*), void *putdat,
    ...
    802005f0:	a0c78793          	addi	a5,a5,-1524 # 80200ff8 <etext+0x5dc>
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap) {
    ...
    8020064c:	9e4a8a93          	addi	s5,s5,-1564 # 8020102c <etext+0x610>
        case 'e':
            err = va_arg(ap, int);
            if (err < 0) {
                err = -err;
    ...
    80200654:	bb8b8b93          	addi	s7,s7,-1096 # 80201208 <error_string>
        while ((ch = *(unsigned char *)fmt ++) != '%') {
    ...
    802008aa:	78260613          	addi	a2,a2,1922 # 80201028 <etext+0x60c>
    ...
    802008bc:	76060613          	addi	a2,a2,1888 # 80201018 <etext+0x5fc>
    ...
    802008ce:	74640413          	addi	s0,s0,1862 # 80201010 <etext+0x5f4>
    ...
    80200958:	6bc40413          	addi	s0,s0,1724 # 80201010 <etext+0x5f4>
            for (; (ch = *p ++) != '\0' && (precision < 0 || -- precision >= 0); width --) {
    ...
    8020096e:	6a7a0a13          	addi	s4,s4,1703 # 80201011 <etext+0x5f5>
